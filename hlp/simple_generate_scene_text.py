...
import sys
import cv2
sys.path.append('/data/zhoujun/tf-crnn/')
...
def write_one(pic_height, to_print_text, fonts, pic_dir, m_count):
    ...
    # pic_width = len(to_print_text) * FONT_SIZE- int(alpha_numeric_number_counter * 0.5 * FONT_SIZE)
    list_paths = []
    list_labels = []
    ...
    background = Image.open(os.path.join(gallery_dir, backgrounds[random_background_num]))
        # .rotate(random_background_rotate_degree,expand=1)
    for i in range(len(fonts)):
        try:
            # 获取字体
            m_font = ImageFont.truetype(os.path.join(font_dir, fonts[i]), FONT_SIZE)
            # crop background as the synthText background randomly 随机剪裁背景
            ...
            if fonts[i].__contains__('msyh'):
                img = img.crop((0,BORDER_SIZE,pic_width,pic_height))
            else:
                img = img.crop((0, 0, pic_width, pic_height-BORDER_SIZE))
            # img = img.resize(shape, Image.BILINEAR)
            # 随机增加黑点噪声
            img = np.array(img.convert('L'))
            if random.randint(0, 6) > 3:
                img = skimage.util.random_noise(img, mode='pepper', amount=0.008)
                # img = matrix2image(img)
            # # 随机增加白点噪声
            # if random.randint(0, 6) > 3:
            #     img = 1 - img
            #     img = skimage.util.random_noise(img, mode='pepper', amount=0.01)
            #     img = 1 - img
            ...
            list_labels.append(''.join([str(Alphabet.CHINESECHAR_LETTERS_DIGITS_EXTENDED.index(x)) + Alphabet.BLANK_SYMBOL for x in to_print_text])[:-1])
            # to_return += ('%s %s\n' % (os.path.abspath(file_name), to_print_text))
...
def write(count: int, mode: str, min_len: int, max_len: int, shape: tuple, worker_num: int):
    ...
    for m_count in range(count):
        ...
        # 每次随机选择1个字体
        # to_font = [fonts[random.randint(0, len(fonts) - 1)]]
        # for i in range(3):
        #     to_font.append(fonts[random.randint(0, len(fonts) - 1)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-font_dir','--font_dir', default='fonts', help='the fonts folder location')
    parser.add_argument('-background_dir','--background_dir', default='background', help='the background images folder location')
    parser.add_argument('-font_size','--font_size', default=32, type=int, help='the font size')
    parser.add_argument('-border_size','--border_size', default=4, type=int, help='the border of text images')
    parser.add_argument('-trd', '--trd', default=0, type=int, help='text rotate degree limit(clockwise & counter clockwise)')
    parser.add_argument('-charset','--charset', default='dict.txt', help='location of charset file,only one line!!!')
    parser.add_argument('-min_len','--min_len', default=3, type=int, help='the min length of text in generated images ')
    parser.add_argument('-max_len','--max_len', default=10, type=int, help='the max length of text in generated images ')
