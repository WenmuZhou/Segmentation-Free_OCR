...
def padding_inputs_width(image: tf.Tensor, target_shape: Tuple[int, int], increment: int) -> Tuple[
    ...
    def pad_fn():
        with tf.name_scope('mirror_padding'):
            ...
            pad_image = tf.pad(img_resized, paddings, mode='SYMMETRIC')
...
def preprocess_image_for_prediction(fixed_height: int = 32, min_width: int = 8,image_channels: int = 3):
    """
    ...
    def serving_input_fn():
        ...
        image = tf.placeholder(dtype=tf.float32, shape=[None, None, image_channels])
